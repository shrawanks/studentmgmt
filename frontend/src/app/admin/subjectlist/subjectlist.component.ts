...
export class SubjectlistComponent implements OnInit {
  ...
  deleteSubject(decision: number) {
    if (decision === 1) {
      this.subjectService.deleteSubject(this.delId).subscribe(
        (response) => {
          console.log(response);
  ...
  updateSubject() {
    this.subjectService.updateSubject(this.editId, this.subject).subscribe(
      (response) => {
